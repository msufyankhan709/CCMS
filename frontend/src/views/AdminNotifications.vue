...
const handleNotificationClick = async (notification) => {
  ...
  if (notification.relatedEntityId) {
    router.push({ path: '/admin/complaints', query: { complaintId: notification.relatedEntityId } })
  }
}
...
const toggleReadStatus = async (notification) => {
  ...
  // Note: We don't have an API endpoint to mark as unread, so we only mark as read
}
